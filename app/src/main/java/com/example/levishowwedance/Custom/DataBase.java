 ...
 */
    ...
    public void insertarFoto(Foto nueva){
        ...
        Cursor cursor=db.rawQuery("SELECT * FROM "+TABLA_FOTOS,null);
        if(cursor.moveToFirst()) {
            for (int i=0;i<5;i++) {
                String data = cursor.getString(i);
                String column_name = cursor.getColumnName(i);
                Log.d("CREATE",column_name );
            }
        }
    ...
    public ArrayList<Foto> buscarFotosUsuario(String username){
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor=db.rawQuery("SELECT * FROM "+TABLA_FOTOS+" WHERE "+USERNAME+" = ? ",null);
        if(cursor.moveToFirst()) {
            foto = new String[6];
            for (int i = 0; i < 6; i++){
                foto[i]=cursor.getString(i);
            }
            Foto nueva = new Foto(foto[0], foto[1], foto[2],foto[3],foto[4]);
            ...
            while (cursor.moveToNext()){
                ...
                nueva = new Foto(foto[0], foto[1], foto[2],foto[3],foto[4]);
                busquedaFotos.add(nueva);
            }
        }
